...
export default <ComponentOptions<Vue>>{
  methods: {
    slice(index: number) {
      (this as any).api.dynamicModals.splice(index, 1)
      this.$nextTick(() => {
        this.$root.$emit('modal:closed', (this as any).api)
      })
    },
    beforeOpen(e: any, modal: any, index: number) {
      ...
      this.$nextTick(() => {
        if (!modal.value) {
          (this as any).slice(index)
          modal.reject('show')
        }
        this.$root.$emit('modal:opened', (this as any).api)
      })
    },
    isString(val: any) {
